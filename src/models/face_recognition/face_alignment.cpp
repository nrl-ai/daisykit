...
static void compute_affine_matrix(float cov[][2], float sigma,
                                  float trans_m[][2]) {
  float u[2][2];
  float w[2];
  float v[2][2];
...
static int get_probe_vec(float* landmark, int landmark_number, float* probe_vec,
...
int cal_scale_and_angle(float* landmark, int landmark_number, int desired_size,
...
int get_aligned_face(cv::Mat& img, float* landmark, int landmark_number,
  ...
  if (cal_scale_and_angle(landmark, landmark_number, desired_size, &scale,
                          &angle) < 0) {
    return -1;
  }
