...
namespace models {
    ...
    class FakeRealClassifiers : public NCNNModel, public ImageModel {
        public: 
            FakeRealClassifiers(const char*param_buffer, const unsigned char*weight_buffer, 
                                int input_width = 480, int input_height = 640, bool use_gpu = false);
            FakeRealClassifiers(const std::string& param_file, const std::string& weight_file,
                                int input_width = 480, int input_height = 640, bool use_gpu = false);
            ...
            float Detect(const cv::Mat&image, std::vector<daisykit::types::FaceExtended> &boxes);
        private:
